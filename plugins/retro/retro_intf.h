...
///
DLL_EXPORT bool retro_intf_init(char const* corePath, char const* gamePath);

/// De-initialize libretro loaded core.
DLL_EXPORT void retro_intf_deinit(void);
...
/// of frames.
DLL_EXPORT size_t retro_intf_drain_audio_buffer(void);

/// Get the audio sample rate.
DLL_EXPORT double retro_intf_get_audio_sample_rate(void);

/// Get the current video configuration.
DLL_EXPORT  retro_intf_video_desc_t retro_intf_get_video_desc(void);

/// Set the controller mapping.
///
/// @param port
/// The input slot; note that only 0 is supported right now and other ports are ignored.
DLL_EXPORT  void retro_intf_set_input(int port, int type, int id);
...
/// @param len
/// Length of |buf|.
DLL_EXPORT  void retro_intf_set_audio_buffer(int16_t* dst, size_t len);
...
DLL_EXPORT  void retro_intf_set_video_buffer(uint8_t* dst);

/// Set the callback for inputs polling.
///
/// NOTE:
/// The callback is triggered during call to retro_intf_step().
DLL_EXPORT void retro_intf_set_input_callback(void (*cb)(input_state_t *));

/// Step the core once.
DLL_EXPORT void retro_intf_step(void);
